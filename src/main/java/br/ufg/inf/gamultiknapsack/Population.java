...
public class Population {
    ...
    public Population twoPointCrossover(Double crossoverRate) {
        // Here, we need to recombine members to generate offspring and add it to population
        Random rn = new Random();
        ArrayList<Member> offspringPopulation = new ArrayList<>();
        ArrayList<Member> parentPopulation = this.getPopMembers();
        for (int i = 0; i < parentPopulation.size() / 2; i++) {
            //Randomly looking for parents to apply 2 point crossover
            int parent1 = rn.nextInt(0, parentPopulation.size() - 1);
            int parent2 = rn.nextInt(0, parentPopulation.size() - 1);
            StringBuilder parentA = new StringBuilder(parentPopulation.get(parent1).getChromosome());
            StringBuilder parentB = new StringBuilder(parentPopulation.get(parent2).getChromosome());
            ...
            if (chanceToGenOffspring <= crossoverRate) {
                int length = parentA.length();
                int cutPositionFirstPoint = rn.nextInt(0, length - 1);
                int cutPositionSecondPoint = rn.nextInt(0, length - 1);
                while (cutPositionSecondPoint <= cutPositionFirstPoint) {
                    cutPositionSecondPoint = rn.nextInt(0, length - 1);
                }
                String sTailA = parentA.substring(cutPositionFirstPoint, cutPositionSecondPoint);
                String sTailB = parentB.substring(cutPositionFirstPoint, cutPositionSecondPoint);
                StringBuilder tailA = new StringBuilder(sTailA);
                StringBuilder tailB = new StringBuilder(sTailB);

                StringBuilder firstTailA = new StringBuilder(parentA.substring(0, cutPositionFirstPoint));
                StringBuilder endTailA = new StringBuilder(parentA.substring(cutPositionSecondPoint, parentA.length() - 1));
                StringBuilder firstTailB = new StringBuilder(parentB.substring(0, cutPositionFirstPoint));
                StringBuilder endTailB = new StringBuilder(parentB.substring(cutPositionSecondPoint, parentB.length() - 1));

                StringBuilder offSpringA = firstTailA.append(tailB).append(endTailA);
                StringBuilder offSpringB = firstTailB.append(tailA).append(endTailB);

                Member offspringA1 = new Member(offSpringA);
                Member offspringB1 = new Member(offSpringB);
                offspringPopulation.add(offspringA1);
                offspringPopulation.add(offspringB1);
            } else {
                // Out the Crossover rate
